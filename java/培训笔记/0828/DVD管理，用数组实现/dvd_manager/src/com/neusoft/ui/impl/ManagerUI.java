...
public class ManagerUI implements IManagerUI {
	...
	public void query() {
		...
		DVD[] dvd = service.queryDVDs();
		for(int i=0; i<dvd.length; i++){
			System.out.print(dvd[i].getId() + "\t");
			System.out.print(dvd[i].isState() ? "已借出\t" : "可借\t");
			System.out.print("《" +dvd[i].getName()+ "》\t\t\t");
			//处理日期
			Date time = dvd[i].getTime();
			if(time != null){
	...
	public void add() {
		...
		DVDContext.id++;
		DVD dvd = new DVD(DVDContext.id, name, false, null, 0, false);
		if(service.save(dvd)){
	...
	public void borrow() {
		...
		do{
			try{
				...
				SimpleDateFormat sf = new SimpleDateFormat("yyyy-MM-dd");
				Date time = sf.parse(strTime);
				//上方没异常，就是输入对了,就name和time有用
				DVD dvd = new DVD(0, name, false, time, 0, false);
